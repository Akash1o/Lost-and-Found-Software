...
const DashboardFound = ({updateFoundCount}) => {
  ...
  const handleDelete=async (id) => {
    ...
    if (!confirmDelete) return;
      ...
      if(response.data.success){
        setFoundData((prevData) => prevData.filter(item => item.found_id !== id));
        updateFoundCount(foundData.length-1);
      }
      else{
  ...
  useEffect(() => {
    axios.get('http://localhost/backend/FetcheData.php')
      .then((response) => {
        if (response.data.success && response.data.foundItem) {
          setFoundData(response.data.foundItem);
          updateFoundCount(response.data.foundItem.length);
        } else {
        ...
        <table className="w-full table-auto border-collapse border border-gray-300">
          ...
          <tbody>
            {foundData && foundData.map((item) => (
              <tr className="border-b hover:bg-gray-100" key={item.found_id}>
                <td className="px-4 py-2">{item.name}</td>
                <td className="px-4 py-2">{item.item}</td>
                <td className="px-4 py-2">{item.location}</td>
                <td className="px-4 py-2">
                  <button className='bg-red-600 hover:bg-red-400 text-white px-2 py-1 rounded-md' onClick={()=>handleDelete(item.found_id)}>Delete</button>
                  <button 
                    className='bg-green-600 hover:bg-green-400 text-white px-2 py-1 rounded-md ml-[10px]' 
                    onClick={() => handlePhoto(item.photoPath)}>View Photo
                  </button>
                </td>
              </tr>
            ))}
